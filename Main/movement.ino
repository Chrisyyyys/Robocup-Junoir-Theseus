...
ForwardMoveResult fwd(double dist){ // in mm
  ...
  double gyroMotionIntegral = 0.0;
  unsigned long lastGyroSampleMs = millis();
  ...
  while((encoderCountA<= pulses && (encoderCountB <= pulses||climbtoggle == true))&&black!=true){
    ...
    */
    ...
    double dt = (nowMs - lastGyroSampleMs) / 1000.0;
    lastGyroSampleMs = nowMs;
    double filteredAccel = abs(myGyro.get_filtered_acceleration());
    gyroMotionIntegral += filteredAccel * dt;

    if (gyroMotionIntegral <= ZERO_MOTION_INTEGRAL_THRESHOLD &&
